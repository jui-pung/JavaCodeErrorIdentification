...
from keras.layers import Dense, Input, GlobalMaxPooling1D,Activation,Dropout, LSTM
...
with open(root_path+'train.txt', encoding="utf8") as f:
    readdata = f.readlines()
...
encoder_outputs, state_h, state_c = LSTM(hidden_latent_dim, return_state=True, return_sequences=False)(embedded_sequences)
   

x = Dense(2*labels_size)(state_h)#softmax
...
with open(root_path+'model_LSTM256_One2One_unweight_73_15', 'wb') as f:
    pickle.dump(model_to_save, f)

y=model.predict(x_test_sequences, batch_size=1024, verbose=1)
